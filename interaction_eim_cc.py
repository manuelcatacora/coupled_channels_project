...
class InteractionEIM_CC(Interaction_CC):
    def __init__(self, coordinate_space_potential_spherical: Callable[[float, np.array],float],    # radial potential function V(r,alpha)
        ...
        if deformed_complex:
            snapshots_sph = np.array([self.radial_potential_sph(self.r_mesh, theta) for theta in self.training_parameters]).T
            
            training_parameters_def = np.array([*self.training_parameters.T[:3],*self.training_parameters.T[6:9]]).T
            snapshots_def = np.array([self.radial_potential_def(self.r_mesh, theta, delta_def) for theta in training_parameters_def]).T
        
        else: 
            snapshots_sph = np.array([self.radial_potential_sph(self.r_mesh, theta) for theta in self.training_parameters]).T
            
            training_parameters_def = self.training_parameters.T[:3].T
            #print(training_parameters_def)
            snapshots_def = np.array([self.radial_potential_def(self.r_mesh, theta, delta_def) for theta in training_parameters_def]).T
    ...
    def coefficients(self,
        ...
        if deformed_complex:
            ...
            u_true_def = self.radial_potential_def(self.r_i_def, alpha_def, delta_def)
    ...
    def radial_potential_emu(self,
        ...
        '''
        a_sph, a_def = self.coefficients(alpha,delta_def, deformed_complex)
